...
class LR:
    def __init__(self, train_set, train_labels, size, step):
        ...
        # 将每句文本中的词向量线性相加生成句子向量，组合训练集所有句子向量生成矩阵
        self.matrix = []
    ...
    def classification(self, test_set, size, numiter, module_file_name):
        labels = []
        # 使用Word2Vec生成测试集数据的词向量
        model = word2vec.Word2Vec.load(module_file_name)
        test_word_vector = list(model.wv.vocab.keys())
        # 使用梯度下降法迭代出w的估计值
        self.weights = self.grandascent(np.array(self.matrix), self.train_labels, numiter)
        # 生成测试集数据的句子向量，再组合成矩阵
        test_x = []
        for line in test_set:
        ...
        # 计算P(y|x)，选择概率最大的项作为测试集数据的类别
        for i in range(len(test_x)):
            labels.append(self.sigmod(test_x[i, :] * self.weights)[0, 0] > 0.5)
        return labels
...
if __name__ == '__main__':
    ...
    '''
    # 对测试集数据进行预测
    nnb = Naive_Bayes(train_set, train_labels)
    results = nnb.classification(test_set)
