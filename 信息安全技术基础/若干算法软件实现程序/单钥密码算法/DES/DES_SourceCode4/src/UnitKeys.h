...
unsigned int GetRandKey();   //生成一字节随机密钥
void Encrypt(const char *Src, char *Dest);    //加密过程
void DeEncrypt(const char *Src, char *Dest);    //解密过程
void DES(const char *Src, char *Dest, bool flag);    //DES算法
void BytesToBits(const char* S, char *bits, int len);   //字节子密钥转成64字节数数形式
void BitsToBytes(char* S, const char *bits, int len);   //64字节数数形式转成字节子密钥
void Transform(const char *Table, char *In, char *Out, int len);   //应用置换
void RotateL(char *In, char *Out, int len, int loop);   //循环左移Loop位
void MakeSubKey(char* In);   //生成16个子密钥
void Xor(char *InA, const char *InB, int len);   //异或
void F_func(char *In, char *Out, const char *Ki);   //F加密函数
void S_func(char *Out, const char *In);   //S-BOX转换
void GenIPTable();   //随机生成IP表及相应的逆表IPR表
bool SetIPTable(const char* In);   //用指定的数组设定IP表
...
const static char KeyTable[16][48] = {
...
25, 1,49,10,35, 3, 19,43,17,60,34,57, 18,50,41,11,59,44,  9,52,51,42,33,27, 39,14,21, 4,54,53, 29,47,22, 7, 5,46, 15,38,55,45,28, 6, 62,31,30,12,37,13,
 9,50,33,59,19,52,  3,27, 1,44,18,41,  2,34,25,60,43,57, 58,36,35,26,17,11, 23,61, 5,55,38,37, 13,31, 6,54,20,30, 62,22,39,29,12,53, 46,15,14,63,21,28,   //12
...
static char Tmp[256], SubKeys[16][49], CD[17][57], LR[17][65], dest[65], DesLoop=16;
