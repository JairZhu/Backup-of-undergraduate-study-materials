...
string eclosure(string sstate) {
	string oldstate = sstate, newstate = sstate;
	for (int i = 0; i < sstate.size(); ++i) {
		for (int j = 0; j < mapping_row; ++j)
			if (mapping[j][0] == sstate[i] && mapping[j][2] == 'k' && newstate.find(mapping[j][1]) == string::npos)
				newstate += mapping[j][1];
	}
	while (newstate != oldstate) {
		oldstate = newstate;
		newstate = eclosure(newstate);
	}
	return newstate;
}
...
void createnewstate(string newstate) {
	...
	for (int k = 0; k < n; ++k) {
		...
		for (int i = 0; i < newstate.size(); ++i) {
			for (int j = 0; j < mapping_row; ++j) 
				if (mapping[j][0] == newstate[i] && mapping[j][2] == str[k] && newstringstate.find(mapping[j][1]) == string::npos) 
					newstringstate += mapping[j][1];	
		}
		newstringstate = eclosure(newstringstate);
