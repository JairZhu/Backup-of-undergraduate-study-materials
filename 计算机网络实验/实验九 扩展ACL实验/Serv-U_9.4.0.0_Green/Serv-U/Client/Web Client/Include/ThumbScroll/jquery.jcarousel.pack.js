...
jQuery.BuildThumbScroll = function(ImagesList,fImageLoad, bShown){
	...
	for (var nIndex = 0; nIndex < anRows.length; nIndex++) {
		...
		// do we have an image
		if (parseInt(ImagesList.getCellValue(5, anRows[nIndex])) == 1) {			
			// get the info from the list	
			sFilePath = ImagesList.getCellText(3, anRows[nIndex]);
			sFileName = ImagesList.getCellText(0, anRows[nIndex]);
			sFileSize = ImagesList.getCellText(1, anRows[nIndex]);
	...
	if(g_ThumbScroll != undefined)
		jQuery.ResetThumbScroll(bShown);
	else {
		 jQuery('#ImageScroller').jcarousel({
			size: g_nScrollSize, scroll: nScrollViewSize,
...
jQuery.ResetThumbScroll = function (bShown) { 
	g_bShown = bShown;
	g_ThumbScroll.reset();
}; // ResetThumbScroll
